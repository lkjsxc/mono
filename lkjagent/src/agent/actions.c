...
result_t agent_actions_save_memory(pool_t* pool, agent_t* agent) {
    ...
    if (string_create(pool, &memory_json) != RESULT_OK) {
        RETURN_ERR("Failed to create string for memory JSON");
    }
    
    if (object_tostring_json(pool, &memory_json, agent->data) != RESULT_OK) {
        if (string_destroy(pool, memory_json) != RESULT_OK) {
            RETURN_ERR("Failed to destroy memory JSON string after conversion failure");
        }
        RETURN_ERR("Failed to convert agent memory to JSON");
    }
    
    // Write to memory file
    if (file_write(MEMORY_PATH, memory_json) != RESULT_OK) {
        if (string_destroy(pool, memory_json) != RESULT_OK) {
            RETURN_ERR("Failed to destroy memory JSON string after write failure");
        }
    ...
    // Clean up
    if (string_destroy(pool, memory_json) != RESULT_OK) {
        RETURN_ERR("Failed to destroy memory JSON string after save");
    }
