...
result_t agent_prompt_append_base(pool_t* pool, string_t** dst, object_t* config_agent_state_base_prompt) {
    ...
    if (object_tostring_xml(pool, &tmp_string, config_agent_state_base_prompt) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after base XML conversion error\n");
        }
    ...
    if (string_escape(pool, &tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after base escape error\n");
        }
    ...
    if (string_append_string(pool, dst, tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after base append error\n");
        }
...
result_t agent_prompt_append_state(pool_t* pool, string_t** dst, object_t* config_agent_state_main_prompt) {
    ...
    if (object_tostring_xml(pool, &tmp_string, config_agent_state_main_prompt) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after state XML conversion error\n");
        }
    ...
    if (string_escape(pool, &tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after state escape error\n");
        }
    ...
    if (string_append_string(pool, dst, tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after state append error\n");
        }
...
result_t agent_prompt_append_memory(pool_t* pool, string_t** dst, object_t* agent_workingmemory) {
    ...
    if (object_tostring_xml(pool, &tmp_string, agent_workingmemory) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after memory XML conversion error\n");
        }
    ...
    if (string_escape(pool, &tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after memory escape error\n");
        }
    ...
    if (string_append_str(pool, dst, "<working_memory>") != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after memory opening tag append error\n");
        }
    ...
    if (string_append_string(pool, dst, tmp_string) != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after memory content append error\n");
        }
    ...
    if (string_append_str(pool, dst, "</working_memory>") != RESULT_OK) {
        result_t tmp = string_destroy(pool, tmp_string);
        if (tmp != RESULT_OK) {
            printf("Warning: Failed to destroy tmp_string after memory closing tag append error\n");
        }
