...
result_t lkjagent_step(pool_t* pool, config_t* config, agent_t* agent) {
    ...
    if (string_destroy(pool, content_type) != RESULT_OK) {
        RETURN_ERR("Failed to destroy content type string");
    }
    if (string_destroy(pool, recv_http_string) != RESULT_OK) {
        RETURN_ERR("Failed to destroy HTTP response string");
    }
    if (object_destroy(pool, recv_http_object) != RESULT_OK) {
        RETURN_ERR("Failed to destroy HTTP response object");
    }
