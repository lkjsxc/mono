...
import { get_path_root, get_relative_path, get_value_at_path } from '../util/json';

/**
 * Handle get action for both working_memory and storage
 */
export async function handle_get_action(
  action: tool_action,
  ...
  try {
    ...
    const value = get_value_at_path(target_data, relative_path);
    
    if (value !== undefined) {
      result.status = 'success';
      result.data = value;
      result.message = `Retrieved data from ${action.target_path}`;
    } else {
      result.error = `Path not found: ${action.target_path}`;
    }
  } catch (error) {
